...
import { walletGetWalletAddress, walletSignMessage } from '@/composables/useWalletClient'
...
export const useUserStore = defineStore('AuthStore', {
    ...
    actions: {
        ...
        async getSignerWalletAddress() {
            try {
                const res = await walletGetWalletAddress()
        ...
        async verify() {
            ...
            const signature = await walletSignMessage(this.authData.nonce)
            ...
            try {
                ...
                setCookie('refresh', res.data.refresh, 7)
            } catch (error) {
