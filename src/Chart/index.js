...
function transform(delta, data, maxPoints, currentPrice, startPrice) {
    ...
    /* const yRange = yExtent[1] - yExtent[0];
              yExtent[0] -= yRange / 4;
              yExtent[1] += yRange / 4; */
    const ratioH = 0.5 // actual graph Y range divided by H
...
const setGameHistory = (historyData) => {
    ...
    // console.log("setGameHistory", data)
    gameState = {
        ...data,
        start_price: data.start_price === "None" ? null : data.start_price,
        end_price: data.end_price === "None" ? null : data.end_price,
        round_start_time: new Date(data.round_start_time).getTime(),
        round_end_time: new Date(data.round_end_time).getTime(),
        trade_start_time: new Date(data.trade_start_time).getTime(),
        trade_end_time: new Date(data.trade_end_time).getTime(),
    }
    gameStateChanged()
}
