...
export function Chart(chartSelector) {
    ...
    centrifugeConnection.on("subGame", function (message) {
        // console.log('updateGameState', message.data);
        tradeGameChart.updateState({
            ...message.data,
            round_start_time: new Date(message.data.round_start_time).getTime(),
            round_end_time: new Date(message.data.round_end_time).getTime() - 1000,
            trade_start_time: new Date(message.data.trade_start_time).getTime(),
            trade_end_time: new Date(message.data.trade_end_time).getTime(),
